  ...
  if (playerSelection === computerSelection) {
    whoWon.innerText = 'Draw!';
      ...
      if (computerSelection === 'Paper') {
        ...
        whoWon.innerText = 'You lost! Paper beats Scissors';
        computerScore++;
      }
  }
  if (playerScore === 5 || computerScore === 5) {
    if (playerScore === 5) {
      whoWon.innerText = "Congratulations! You won the game";
    } else {
      whoWon.innerText = "Tough luck! You lost the game";
    }
    rock.disabled = true;
    paper.disabled = true;
    ...
    bottomDiv.appendChild(playAgain);
  }
}
...
function showScore(animate) {
  scoreDiv.innerHTML = 'Score' + `<br />     ` + `You  ` + `${playerScore}-${computerScore} Computer`;
  if (animate === 1) {
